...
The website must work as a reusable aggregator shell where the main browser title, company identity, branding, logo, and contact details can be changed from one small configuration file. Page copy, legal text, service content, labels, and operational settings remain part of the site source.

## 2. Required Pages

This project uses a compact landing-page structure:

- `index.html` - homepage with About, Services, and Contact sections
- `complete-bathroom-remodel.html` - complete-remodel service guide
- `shower-bathtub-remodeling.html` - shower and bathtub service guide
- `privacy.html` - privacy policy
- `terms.html` - terms of service
- `cookie-policy.html` - cookie policy

Primary navigation should link About and Contact to `index.html#about` and `index.html#contact`. The service overview is available at `index.html#services`.
...
Only the main browser title and company-level data belong in this file. Do not place page copy, UI labels, legal settings, form behavior, or service content in it.
...
- Configurable logo image, with the built-in mark as fallback
- Website text and link
- Business hours
- Main browser title and company-name suffix
...
data-company-id
data-company-brand-name
data-company-brand-descriptor
data-email-text
data-email-link
data-website-text
data-website-link
...
The home-page browser title must use `SITE_CONFIG.pageTitle`, and the company-name suffix must use `SITE_CONFIG.company.name`.

## 4. Service Page Content Requirement

Service-page content must be present directly in each service HTML file. JavaScript may provide shared navigation and interactions, but it must not be required to render the service description, included work, options, process, planning details, FAQs, images, or related-service links.
...
Service pages must remain readable and complete if client-side JavaScript is unavailable. Shared company identity and contact details may still be hydrated from `js/site-config.js`.
...
- Pull company name, legal name, and contact email from config
...
The contact section on `index.html` must be visually aligned with the rest of the website and reachable through `index.html#contact`.
...
If using a backend form handler, it should read destination email/company data from `SITE_CONFIG.contact.email` or an equivalent server-side config source, not from hardcoded values.
